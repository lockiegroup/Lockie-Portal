...
<main class="max-w-7xl mx-auto px-4 sm:px-6 py-8">
    ...
    <div style="display:flex;align-items:flex-end;justify-content:space-between;flex-wrap:wrap;gap:12px;margin-bottom:2rem;">
        <div>
            ...
            <p class="text-slate-500 mt-1 text-sm">Ranked by spend in the last 12 months. Comparing against the prior 12 months.</p>
        </div>
    </div>

    {{-- Filters --}}
    <form method="GET" style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:1.5rem;">
        <input type="text" name="search" value="{{ $search }}" placeholder="Search customer or code…"
            style="flex:1;min-width:200px;padding:8px 12px;border:1px solid #e2e8f0;border-radius:8px;font-size:0.875rem;color:#1e293b;background:#fff;outline:none;">
        ...
        @if($search || $warehouse)
            <a href="{{ route('crm.index') }}"
               style="padding:8px 14px;background:#f1f5f9;color:#64748b;border-radius:8px;font-size:0.875rem;text-decoration:none;display:inline-flex;align-items:center;">
                Clear
            </a>
        @endif
    </form>
        ...
        <table style="width:100%;border-collapse:collapse;font-size:0.875rem;">
            <thead>
                <tr style="background:#f8fafc;border-bottom:1px solid #e2e8f0;">
                    ...
                    <th style="padding:10px 16px;text-align:left;font-size:0.7rem;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:0.06em;">Orders</th>
                    <th style="padding:10px 16px;text-align:left;font-size:0.7rem;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:0.06em;width:40px;"></th>
                </tr>
            </thead>
            <tbody>
                @forelse($customers as $c)
                    @php
                        ...
                        $lastOrderCarbon = $c->last_order;
                        $daysSince = $lastOrderCarbon ? $lastOrderCarbon->diffInDays(now()) : null;
                    @endphp
                        ...
                        <td style="padding:12px 16px;">
                            @if($lastOrderCarbon)
                                <span style="color:{{ $daysSince > 180 ? '#dc2626' : ($daysSince > 90 ? '#d97706' : '#64748b') }};font-size:0.875rem;">
                                    {{ $lastOrderCarbon->format('d M Y') }}
                                </span>
                                <div style="font-size:0.75rem;color:#94a3b8;">{{ $lastOrderCarbon->diffForHumans() }}</div>
                            @else
                                <span style="color:#cbd5e1;">—</span>
                            @endif
                        </td>
                        <td style="padding:12px 16px;color:#64748b;">{{ $c->order_count }}</td>
                        <td style="padding:12px 16px;">
                            <a href="{{ route('crm.show', ['customerCode' => $c->customer_code, 'warehouse' => $warehouse]) }}"
                               style="font-size:0.75rem;color:#94a3b8;text-decoration:none;">View &rarr;</a>
                        </td>
                    </tr>
                @empty
                    <tr>
    ...
    <div style="margin-top:1rem;font-size:0.75rem;color:#94a3b8;text-align:right;">
