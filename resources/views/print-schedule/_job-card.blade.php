@php
    ...
    if ($job->line_comment && preg_match('/Numbered:\s*(\d+)\s*[-–]+\s*(\d+)/i', $job->line_comment, $_lm)) {
        ...
        $_lNum['defaultPack'] = $job->order_quantity > 0
            ? (int) round(($_lNum['end'] - $_lNum['start'] + 1) / $job->order_quantity)
            : 100;
    }
@endphp
...
<div class="job-card bg-white rounded-xl border shadow-sm select-none {{ $job->is_manual ? 'border-green-300' : 'border-slate-200' }}" style="padding: 1.25rem 1.5rem;"
    ...
    <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
        ...
        {{-- Download Labels --}}
        <button onclick="openLabelModal(this)"
            data-job-id="{{ $job->id }}"
            data-num-start="{{ $_lNum['start'] ?? '' }}"
    ...
    @once
    {{-- Label download modal (rendered once, shared across all cards) --}}
    ...
    function lmUpdate() {
        ...
        if (numStart && numEnd && numEnd > numStart) {
            ...
            const count = Math.ceil(total / packSize);
            info = `${total.toLocaleString()} items ÷ ${packSize.toLocaleString()} per pack = <strong>${count.toLocaleString()} label${count !== 1 ? 's' : ''}</strong>`;
        } else if (orderQty > 0) {
            const count = Math.ceil(orderQty / packSize);
            info = `${orderQty} packs ÷ ${packSize} = <strong>${count} label${count !== 1 ? 's' : ''}</strong>`;
        } else {
            info = '<strong>1 label</strong> will be generated';
        }
    ...
    function lmDownload() {
        ...
        window.open(`/print-schedule/jobs/${jobId}/labels?branded=${branded}&pack_size=${packSize}`, '_blank');
